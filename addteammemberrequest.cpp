...
void AddTeamMemberRequest::processRequest(QJsonObject request, QTcpSocket *clientSocket)
{
    ...
    if(user_id==-1 or qh.isInTeam(user_id,team_id) or !qh.isAdmin(request_sender_id,team_id))
    {
        resp=new ErrorResponse();
    }
    else
    {
        qh.addUserToTeam(team_id,user_id);
